-- TAs can record late-day claims for students without the self-claim lateness minimum.
-- Balance caps and active-deadline checks still apply.
...
CREATE OR REPLACE FUNCTION public.perform_late_day_claim(
  ...
  p_days integer,
  p_enforce_minimum_days boolean DEFAULT true
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_enforce_minimum_days THEN
...
REVOKE ALL ON FUNCTION public.perform_late_day_claim(text, text, text, uuid, integer, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.perform_late_day_claim(text, text, text, uuid, integer, boolean) TO authenticated;
